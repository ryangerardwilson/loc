...
- Keep no-arg execution as the help surface. Use `loc count all` for today's totals across every configured alias.
...
- `loc token add <alias> [<token>]` is the canonical setup path for saving or replacing aliases.
- `loc count all`, `loc count <alias>`, `loc token add <alias> [<token>]`, and `loc config` are the canonical command forms.
